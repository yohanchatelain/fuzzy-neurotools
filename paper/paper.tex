...
\begin{abstract}
  ...
  precision level found in commonly-used libraries \gk{What does this mean? They only have precision of 17, or they use data that is 17 bits?}. \yohan{agree with Greg, it is unclear and a strong assertion!} Our findings motivate the
...
using similar methods \gk{this sentence says "two factors" but then only mentioned between tool. Also, it is too long}. \yohan{What do you mean by methods? Numerical schemes? If so, are we sure that theses tools are using the same numerical schemes?} Numerical variability, in contrast, denotes
