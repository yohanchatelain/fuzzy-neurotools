...
To validate instrumentations, we verified that two consecutive IEEE executions are identical;
there are no randomness effects in all pipelines.
We also listed all the pipeline library dependencies using the ldd Linux utility.
We ensured that (1) all pipelines are dynamically linked to the libmath library
(2) there is no alternative implementation for the basic mathematical functions in these dependencies.
Finally, we verified that Fuzzy libmath affects computational results.
...
between activation clusters in the 360 regions in the Human
Connectome Project Multi-Modal Parcellation atlas version 1.0
(HCP-MMP1.0)~\cite{glasser2016multi}. For BT, a region was unstable if it had different activation behavior between pair of tools.
For WT, we considered a region unstable if it had a blinking activation between Fuzzy samples of a given tool.
...
SPM or different hardware. We performed visual quality control of the AFNI and SPM results
...
Overall, BT variability was larger than WT variability in both thresholded and unthresholded maps.
...
\begin{table}[h]
    ...
    \begin{tabular}{cccc|cc}
        ...
        \bottomrule
    \end{tabular}
    \caption{Summary statistics of voxel-wise standard deviations
    in group-level t-statistics in BT and WT.}
...
\begin{table}[h]
    ...
    \begin{tabular}{cccc}
        ...
        \bottomrule
    \end{tabular} 
    \caption{Summary statistics of voxel-wise standard deviations in subject-level t-statistics in BT and WT.
    The voxel-wise standard deviations were averaged across 16 subjects.}
...
show significant WT variability, particularly for some regions in the frontal, parietal, and limbic lobes, close to BT variability.
  ...
  (Figure~\ref{fig:thresh-maps}).
  Figure~\ref{fig:cm-thresh} compares instability of regional activations,
  showing that most of the regions have a similar activation pattern in both BT and WT, mostly are stable.
  However, some regions have a different activation behavior, mainly unstable in BT and stable in WT.
  % showing more severity of BT instability than WT.
  ...
  \begin{figure*}[ht]
      ...
      (\textbf{B}) as defined in Eq~\ref{eq:wt-pair}.}
    ...
    \begin{figure*}[ht]
      ...
      \caption{Heatmap of confusion matrices for instability of regional activations
      in BT and WT using the HCP-MMP1.0 parcellation of ~\cite{glasser2016multi}. 
      A region is unstable (=1) if it has different activation behavior in both tools
      for BT or different fuzzy samples in each tool for WT; otherwise, it is stable (=0).}
      \label{fig:cm-thresh}
    \end{figure*}
