...
reduce the time overhead resulting from Fuzzy Libmath instrumentation \gk{Did we verify that single-threading generally didn't lead to different results than 7-threading? As in, were they all ostensibly sampled from the same random distribution?}. All
...
Canada's} Béluga cluster nodes, each with 2$\times$ Intel Gold 6148 Skylake~@~2.4~GHz
 ...
 we encapsulated the
above-mentioned software packages in Docker\gk{CC doesn't support Docker, so you mean Singularity?} container images based on CentOS 7.3.
...
and unthresholded maps (Wilcoxon signed-rank test p~\textless~$10^{-162}$ \gk{we don't have sufficient sample size to say that p is that small, so we should probably say less than $10^{-5}$ or something else we could actually arrive at with our number of repetitions, pipeline pairs, and samples.}
...
\begin{figure*}[ht]
  ...
  signed-rank test p~\textless~$10^{-160}$\gk{pval comment} for all tests, t-test p=0 for
