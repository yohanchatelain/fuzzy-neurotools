...
signed-rank test p=0 for all tests, t-test p~\textless~$10^{-163}$ for all
tests). 
...
\begin{figure*}[ht]
    ...
    and difference between them (\textbf{C}). \tristan{Add XY plots for each map}}
  ...
  subjects).
  \tristan{Add the subject XY plots as supplementary material}
...
that BT and WT reach comparable magnitudes in some regions.
  ...
  \begin{figure}[ht]
    \fbox{\begin{minipage}{\dimexpr \textwidth-2\fboxsep-2\fboxrule}
        ...
        WT is computed at the virtual precision of t=17 bits.\tristan{merge with figure with brain maps}}
    \label{fig:gnp-mni}
    \end{minipage}}
  \end{figure}
...
\tristan{Add SPM and AFNI replication figure here.}
...
\begin{figure*}[ht]
  \fbox{\begin{minipage}{\dimexpr \textwidth-2\fboxsep-2\fboxrule}
    ...
    \end{subfigure}
    \caption{
      Voxel-wise comparison of BT and WT variabilty subject by subject. \tristan{update figure}}
